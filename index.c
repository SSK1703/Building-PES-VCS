...
int index_load(Index *index) {
    ...
    if (!f) {
        // Index file doesn't exist yet - this is normal for a new repo
        return 0;
    }

    char mode_str[32], hash_hex[HASH_HEX_SIZE + 1];
    uint64_t mtime_sec;
    uint32_t size;
    char path[512];

    while (index->count < MAX_INDEX_ENTRIES) {
        int n = fscanf(f, "%32s %64s %" PRIu64 " %" PRIu32 " %511s\n",
                       mode_str, hash_hex, &mtime_sec, &size, path);
        if (n != 5) break;  // End of file or parse error

        IndexEntry *entry = &index->entries[index->count];
        entry->mode = (uint32_t)strtoul(mode_str, NULL, 8);  // Octal
...
int index_save(const Index *index) {
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        if (fprintf(f, "%05o %s %" PRIu64 " %" PRIu32 " %s\n",
                    sorted[i].mode, hash_hex, sorted[i].mtime_sec,
...
int index_add(Index *index, const char *path) {
    ...
    entry->mode = 0100644;  // Regular file, 644 permissions
    entry->hash = hash;
